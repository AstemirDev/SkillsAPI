...
public class CatmullRomSpline{
    ...
    private final Vector3[] pts;

    public CatmullRomSpline(Vector3[] pts) {
        this.pts = new Vector3[pts.length];
        System.arraycopy(pts, 0, this.pts, 0, pts.length);
    }

    public Vector3 interpolate(float s) {
        int numSections = pts.length - 3;
        int currPt = (int)Math.min(Math.floor(s * (float) numSections), numSections - 1);
        Vector3 P0 = this.pts[currPt];
        Vector3 P1 = this.pts[currPt + 1];
        Vector3 P2 = this.pts[currPt + 2];
        Vector3 P3 = this.pts[currPt + 3];
        float t0 = 0.0F;
    ...
    public Vector3 derivative(float s) {
        int numSections = pts.length - 3;
        int currPt = (int)Math.min(Math.floor(s * (float) numSections), numSections - 1);
        ...
        Vector3 dC = new Vector3(dCx, dCy, dCz);
    ...
    private float getT(float t, Vector3 p0, Vector3 p1) {
        ...
        //alpha = 1.0F: chordal
        float alpha = 0.5F;
        return (float)Math.pow(p1.sub(p0).length(), alpha) + t;
    }
